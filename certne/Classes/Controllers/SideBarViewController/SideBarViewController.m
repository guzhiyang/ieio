...
- (void)viewDidLoad
{
    ...
    //--注册通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(moveAnimationWithSender:) name:SidebarShowNotification object:nil];
}
...
-(void)testNotification:(id)sender
{
    ...
    NSLog(@"回传的参数:%@",notification.object);
    NSLog(@"接收到通知了！");
}
...
#pragma mark - SidebarMoveAnimation notification methods

-(void)moveAnimationWithSender:(id)sender
{
    NSNotification *notification = (NSNotification *)sender;
    ...
    void (^animations)(void) = ^{
        if ([notification.object isEqualToString:@"0"]) {
            ...
            self.contentView.transform = CGAffineTransformMakeTranslation(ContentMinOffset, 0);
        }
    };
    ...
    self.contentView.userInteractionEnabled = NO;//--清除和用户交互的事件队列（包括点击事件和键盘）
    self.navBackView.userInteractionEnabled = NO;
    [UIView animateWithDuration:MoveAnimationDuration animations:animations completion:complete];//--UIView 指定执行时间执行动画 完成后指定一个结果
}
