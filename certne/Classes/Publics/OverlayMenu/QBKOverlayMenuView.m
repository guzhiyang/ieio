...
#define QBK_OVERLAY_MENU_MAX_ADDITIONAL_BUTTONS 5               // 控制按钮个数
#define QBK_OVERLAY_MENU_ADDITIONAL_BUTTONS_WIDTH 30            // 按钮的宽度
#define QBK_OVERLAY_MENU_ADDITIONAL_BUTTONS_HEIGHT 30           // 按钮的高度
...
#define QBK_OVERLAY_MENU_VIEW_MAIN_BUTTON_WIDTH 30              // 旋转按钮的宽度
#define QBK_OVERLAY_MENU_VIEW_MAIN_BUTTON_HEIGHT 30             // 旋转按钮的高度
...
- (void)setParentView:(UIView *)view
{
    ...
    _mainBackgroundImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"overlay_menu.png"]];
...
- (void)unfoldWithAnimationDuration:(float)duration
{
    [UIView animateWithDuration:duration animations:^{
        ...
        [_mainButton setBackgroundImage:[UIImage imageNamed:@"arrow_up.png"] forState:UIControlStateNormal];
...
- (void)addButtonWithImage:(UIImage *)image index:(NSInteger)index
{
    ...
    CGFloat tamHuecoBoton = (320 - QBK_OVERLAY_MENU_VIEW_WIDTH) / QBK_OVERLAY_MENU_MAX_ADDITIONAL_BUTTONS;
    
    CGFloat posXBotonCentrado = (tamHuecoBoton - QBK_OVERLAY_MENU_ADDITIONAL_BUTTONS_WIDTH) / 2;
    CGFloat posYBotonCentrado = (([_contentView bounds].size.height - QBK_OVERLAY_MENU_ADDITIONAL_BUTTONS_HEIGHT) / 2);
...
- (void)setupMainButton
{
    ...
    CGFloat y = (QBK_OVERLAY_MENU_VIEW_HEIGHT - QBK_OVERLAY_MENU_VIEW_MAIN_BUTTON_HEIGHT) / 2;
...
- (CGRect)createFoldedMainFrameForPosition:(QBKOverlaMenuViewPosition)position
{
    ...
    switch (_position) {
        case kQBKOverlayMenuViewPositionBottom:
            frame = CGRectMake([_parentView bounds].size.width - QBK_OVERLAY_MENU_VIEW_WIDTH, [_parentView bounds].size.height - (QBK_OVERLAY_MENU_VIEW_HEIGHT), QBK_OVERLAY_MENU_VIEW_WIDTH, QBK_OVERLAY_MENU_VIEW_HEIGHT);
            break;
        case kQBKOverlayMenuViewPositionTop:
            frame = CGRectMake([_parentView bounds].size.width - QBK_OVERLAY_MENU_VIEW_WIDTH, _offset.topOffset, QBK_OVERLAY_MENU_VIEW_WIDTH, QBK_OVERLAY_MENU_VIEW_HEIGHT);
            break;
        default:
            frame = CGRectMake([_parentView bounds].size.width - (QBK_OVERLAY_MENU_VIEW_WIDTH), [_parentView bounds].size.height - (QBK_OVERLAY_MENU_VIEW_HEIGHT + _offset.bottomOffset), QBK_OVERLAY_MENU_VIEW_WIDTH, QBK_OVERLAY_MENU_VIEW_HEIGHT);
            break;
    }
...
- (CGRect)createUnfoldedMainFrameForPosition:(QBKOverlaMenuViewPosition)position
{
    ...
    switch (_position) {
        case kQBKOverlayMenuViewPositionBottom:
            frame = CGRectMake( 0, [[self superview] bounds].size.height - (QBK_OVERLAY_MENU_VIEW_HEIGHT), [[self superview] bounds].size.width, QBK_OVERLAY_MENU_VIEW_HEIGHT);
            break;
        case kQBKOverlayMenuViewPositionTop:
            frame = CGRectMake( 0, _offset.topOffset, [[self superview] bounds].size.width, QBK_OVERLAY_MENU_VIEW_HEIGHT);
            break;
        default:
            frame = CGRectMake( 0, [[self superview] bounds].size.height - (QBK_OVERLAY_MENU_VIEW_HEIGHT + _offset.bottomOffset), [[self superview] bounds].size.width, QBK_OVERLAY_MENU_VIEW_HEIGHT);
            break;
    }
