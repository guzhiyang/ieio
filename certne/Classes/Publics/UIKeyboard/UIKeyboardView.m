...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        UIImage *barItemImage = [UIImage imageNamed:@"button_green.png"];
        barItemImage=[barItemImage stretchableImageWithLeftCapWidth:5 topCapHeight:5];
        
		keyboardToolbar = [[UIToolbar alloc] initWithFrame:frame];
		keyboardToolbar.barStyle = UIBarStyleBlackTranslucent;
		
		UIBarButtonItem *previousBarItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"上一步", @"")
																			style:UIBarButtonItemStylePlain
																		   target:self
																		   action:@selector(toolbarButtonTap:)];
		previousBarItem.tag = 1;
        [previousBarItem setBackgroundImage:barItemImage forState:UIControlStateNormal barMetrics:UIBarMetricsDefault];
		
		UIBarButtonItem *nextBarItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"下一步", @"")
																		style:UIBarButtonItemStyleBordered
																	   target:self
																	   action:@selector(toolbarButtonTap:)];
		nextBarItem.tag = 2;
        [nextBarItem setBackgroundImage:barItemImage forState:UIControlStateNormal barMetrics:UIBarMetricsDefault];
