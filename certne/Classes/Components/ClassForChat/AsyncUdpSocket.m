...
#define SENDQUEUE_CAPACITY	  5   // 发送队列的初始容量
#define RECEIVEQUEUE_CAPACITY 5   // 接收队列的初始容量

#define DEFAULT_MAX_RECEIVE_BUFFER_SIZE 9216  //--默认的接收缓冲区的最大值

NSString *const AsyncUdpSocketException = @"AsyncUdpSocketException";//-－extern 表示这里定义的值在.h的定义里可以使用
				...
				if(theSocket == theSocket4)
				{
					...
					if(result >= 0)
					{
						...
						else
						{
                            ...
                            // theCurrentReceive->buffer 是对象指针theCurrentReceive的成员buffer 作用是选择成员跟theCurrentReceive.buffer（对象的成员）点运算符有区别 作用都是选择成员
